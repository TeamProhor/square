...
export default function NewBatchPage() {
  ...
  return (
    <div className="flex flex-col w-full max-w-2xl mx-auto pb-12 pt-2 md:py-8 gap-8">
      <div>
        <h1 className="text-2xl font-extrabold tracking-tight">নতুন ব্যাচ তৈরি করুন</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5 border p-6 rounded-xl bg-card">
        ...
        <div className="space-y-2">
          ...
          <Input name="name" required placeholder="e.g. HSC 2026 Science" className="rounded-xl" />
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium">স্লাগ (Slug)</label>
          <Input name="slug" required placeholder="e.g. hsc-2026-science" className="rounded-xl" />
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium">বর্ণনা (Optional)</label>
          <Textarea name="description" placeholder="Short description of this batch" className="rounded-xl min-h-[100px]" />
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium">Course ID (Optional)</label>
          <Input name="courseId" placeholder="Related Course ID if any" className="rounded-xl" />
        </div>
