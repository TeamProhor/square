  ...
  return (
    <div className="flex flex-col w-full max-w-5xl mx-auto pb-12 pt-2 md:py-8 gap-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Total Submissions: {submissions?.length || 0}</p>
      </div>
          ...
          <tbody className="divide-y bg-card">
            {submissions?.map((sub: any) => (
              <tr key={sub.id}>
                ...
                <td className="p-3 font-bold">{sub.score} / {sub.totalMarks}</td>
                <td className="p-3">{sub.timeTakenSeconds}s</td>
                <td className="p-3">
                  <span className={`px-2 py-0.5 rounded-full text-[10px] uppercase font-bold ${sub.status === 'submitted' ? 'bg-emerald-500/10 text-emerald-600' : 'bg-amber-500/10 text-amber-600'}`}>
                    {sub.status}
                  </span>
                </td>
                <td className="p-3 text-muted-foreground">{sub.submittedAt ? new Date(sub.submittedAt).toLocaleString() : "N/A"}</td>
              </tr>
            ))}
            {(!submissions || submissions.length === 0) && (
              <tr>
                <td colSpan={5} className="p-8 text-center text-muted-foreground">কোনো ফলাফল পাওয়া যায়নি</td>
              </tr>
            )}
          </tbody>
        </table>
