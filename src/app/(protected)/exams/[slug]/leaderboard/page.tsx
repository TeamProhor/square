        ...
        <table className="w-full text-left text-sm md:text-base">
          <thead className="bg-muted">
            <tr>
              ...
              <th className="p-4 font-bold text-right hidden sm:table-cell">
                Time Taken
              </th>
            </tr>
          </thead>
          <tbody className="divide-y">
            {leaderboard?.map((entry) => {
              ...
              let rankBadge = "";
              if (entry.rank === 1) {
                rankStyle = "text-amber-500 font-black text-xl";
                rankBadge = "👑";
              } else if (entry.rank === 2) {
                rankStyle = "text-slate-400 font-black text-lg";
              } else if (entry.rank === 3) {
                rankStyle = "text-amber-700 font-black text-lg";
              }

              return (
                <tr
                  key={`${entry.userId}-${entry.rank}`}
                  className={`${isMe ? "bg-primary/5" : "bg-card"}`}
                >
                  ...
                  <td className="p-4 text-center font-bold">{entry.score}</td>
