  ...
  return (
    <div className="flex flex-col h-screen w-full bg-muted/30">
      ...
      <div className="h-16 border-b bg-card flex items-center justify-between px-4 sm:px-8 sticky top-0 z-10 shrink-0">
        <h1 className="font-bold text-lg hidden sm:block truncate max-w-sm">
          {exam.title}
        </h1>

        <div
          className={`font-mono font-bold text-xl px-4 py-1.5 rounded-xl ${timeLeft < 300 ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary"}`}
        >
          {formatTime(timeLeft)}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-20 sm:w-64 border-r bg-card flex flex-col shrink-0">
          <div className="p-4 border-b font-semibold text-sm hidden sm:block bg-muted/50">
            Questions
          </div>
          ...
          {exam.examQuestions.map((eq: any, idx: number) => {
            ...
            return (
              <div
                key={eq.id}
                id={`q-${eq.id}`}
                className="bg-card border rounded-2xl p-6 shadow-sm scroll-mt-20"
              >
                ...
                {q.type === "mcq" && q.mcqOptions && (
                  <div className="space-y-3 pl-8">
                    {q.mcqOptions.map((opt: any) => {
                      const isSelected =
                        answers[eq.id]?.selectedOptionId === opt.id;
                      return (
                        <div
                          key={opt.id}
                      ...
                      <div key={part.id} className="space-y-2">
                        <div className="flex justify-between items-start">
                          <span className="text-sm font-semibold text-foreground">
                            ({part.partKey}) {part.questionText}
                          </span>
