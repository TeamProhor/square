...
import { Clock, StatusUp } from "@/components/icons";
...
export default function ResultClient({
  ...
  const percentage =
    (parseFloat(submission.score) / submission.totalMarks) * 100;
  const isPass = exam.passMarks
    ? parseFloat(submission.score) >= exam.passMarks
    : true;
    ...
    <div className="flex flex-col w-full max-w-4xl mx-auto pb-12 pt-4 md:py-8 gap-8 px-4">
      ...
      <div className="bg-card border rounded-3xl p-6 md:p-10 flex flex-col items-center justify-center text-center shadow-sm">
        ...
        <div className="relative size-40 md:size-48 flex items-center justify-center rounded-full border-[12px] border-muted mb-8">
          <div
            className={`absolute inset-0 rounded-full border-[12px] ${isPass ? "border-emerald-500" : "border-destructive"}`}
        ...
        <div
          className={`px-6 py-2 rounded-full font-bold text-lg mb-8 ${isPass ? "bg-emerald-500/10 text-emerald-600" : "bg-destructive/10 text-destructive"}`}
        >
        ...
        <div className="flex gap-4 md:gap-8 w-full justify-center text-sm md:text-base">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="font-bold text-emerald-500">
              {responses.filter((r: any) => r.isCorrect).length} টি
            </span>
          </div>
          <div className="w-px bg-border h-10"></div>
          <div className="flex flex-col items-center">
            ...
            <span className="font-bold text-destructive">
              {
                responses.filter((r: any) => !r.isCorrect && r.selectedOptionId)
                  .length
              }{" "}
              টি
            </span>
      ...
      <div className="space-y-6">
        ...
        {responses.map((res: any, idx: number) => {
          ...
          return (
            <div
              key={res.id}
              className={`bg-card border rounded-2xl p-6 ${isCorrect ? "border-emerald-500/30" : res.selectedOptionId ? "border-destructive/30" : ""}`}
            >
              <div className="flex justify-between items-start gap-4 mb-4">
                ...
                <div
                  className={`text-xs font-bold px-2 py-1 rounded whitespace-nowrap shrink-0 
                      ...
                      >
                        ...
                        >
                          {(isSelected || isActualCorrect) && (
                            <div
                              className={`size-2.5 rounded-full ${isActualCorrect ? "bg-emerald-500" : "bg-destructive"}`}
                            />
                          )}
                        </div>
                        <span
